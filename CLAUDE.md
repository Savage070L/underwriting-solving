...
`index.html` is split into **tab-panel sections** (`.tab-panel#tab-{refs,contracts,daip,contractor,decision}`). The nav (`.tab-nav`) lives **inside the header** (`.app-header-inner`, between the brand and `.app-meta`) — tabs use **SVG icons** (`.tab-btn-ico`), not numbers. It is **not** a multi-page app — all tabs share the in-memory `App` state and switching just shows/hides panels (no reload, nothing serialized). `App.switchTab(name[, scroll])` toggles `.is-active` on the panel + button and persists `localStorage['active_tab']`; `App._initTabs()` (called from `init()`) restores it, defaulting to `refs` when not all 6 refs are loaded, else `decision`. **Display order** (left→right, `App.TABS`) — the panel DOM order differs and doesn't matter:
...
Beyond the Worker, the Chrome extension (`chrome-extension/`, "мост к stat.gov.kz") proxies browser-side sources via `StatGovClient` (page↔extension `postMessage` bridge): `STATGOV_LOOKUP` (stat.gov.kz, needs ЭЦП session — name/OKED/КРП/address/reg date), `STATGOV_HEALTH` (lightweight connectivity probe — Step-1 GET only, no БИН, reports `{reachable, session}`; drives the header `#statgov-pill`), `STATSNET_LOOKUP` (statsnet.co — industry, slow, opens tabs), `KYC_LOOKUP` (**kyc.kz, no auth** — base card parsed from the SSR `window.__NUXT__` IIFE without eval; see `chrome-extension/README.md`), and **`EGOV_RESIDENCY_LOOKUP`** (**egov.kz, needs egov portal session** — `GET /services/P30.11/rest/gbdul/organizations/{bin}` with `credentials:'include'`; the **authoritative residency source**, see the residency section). kyc is the registration-date/address fallback in batch generation. `Utils.statgovLegalAddress(sg)` resolves the legal address from a statgov object, falling back to «Местонахождение …» in `sg._raw` (ИП have no «Юридический адрес»). After changing extension `host_permissions`, the user must reload the unpacked extension.
...
- **Проверка Договоров** — **two separate columns** «Рези-дент (С)» and «Рези-дент (К)», each checked by its own БИН: Страхователь (`_insurerBin(r)`, cell class `batch-c-resident-s`) and Контрагент (`r.bin`, `batch-c-resident-k`). One badge per cell via `BatchAR._residCellFor(bin)` (built on `BatchAR._residBadge(bin)` → `{cls, txt, title}`); values `✓` резидент · `✓ ликв.` · `нерез.` · `ИП` · `н/д`. Toolbar toggle **«🌐 Сначала нерезиденты»** (a `_SORT_DEFS` entry, so the button/active-state render for free via `_renderSortChips`): ranks rows нерезидент(3) > ИП(2) > резидент(1) > н/д(0), where the row rank = `_residRowRank(r)` = **max over both parties** (`_insurerBin(r)` and `r.bin`) so a non-resident on either side floats up; combinable with the other «Сначала …» toggles. Computed synchronously at render time, so it does **not** gate generation (`_verifyComplete`) the way stat.gov.kz does. `renderTable()` re-renders once via `ResidentCheck.onReady` if the index was still loading. The batch table is now **21 columns** — колонки заданы `<colgroup>` в % (fixed layout, no horizontal scroll); adding/removing a column means rebalancing those percentages to sum to 100.
- **«⬇ Скачать результаты проверки (HTML)»** (`#batch-download-html` → `BatchAR.downloadResults`) — exports the whole reviewed registry as **one self-contained HTML report** (opens in any browser, prints to PDF). Visible whenever rows exist; **not gated on stat.gov.kz**. Key detail: the live table is **paginated** (only the current page is in the DOM), so `downloadResults` temporarily sets `_pageSize = rows.length` + `renderTable()` to materialise **all** rows, snapshots the `<table>` HTML, then restores the page — all **synchronously before any `await`** so nothing flickers. The snapshot is cleaned (approve checkbox → static ✓/—, all `onclick`/`oninput` stripped, inputs disabled), then `_buildResultsHtml` wraps it: header (timestamp, source filename `_sourceFileName`, ГБД ЮЛ base date, active sort), a summary chip row (`_resultCounts`: ok/warn/err/approved/unchecked), a **residency chip row** (нерезидент/ИП counts per side), a colour legend, and the full table. `style.css` is **inlined** (fetched at export time) for standalone fidelity + a print block (`@page A4 landscape`). Downloaded via `saveAs` (FileSaver) as `Результаты проверки договоров DD.MM.YYYY (N).html`.
