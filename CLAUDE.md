...
**stat.gov.kz status indicator.** The old «Локальная обработка» pill is replaced by `#statgov-pill` (top-right), driven by `App.STATGOV`. **Ground truth = real lookups**: `autoLookupStatGov` calls `App.STATGOV.noteLookup(ok, err)` — a successful БИН lookup ⇒ `online` (and stamps `_lastLookupOk`); a session-error ⇒ `nologin`. The passive probe `StatGovClient.health()` (the `STATGOV_HEALTH` bridge message — extension does Step-1 GET + checks for the `sessid` hidden input) is only the **fallback** for when no lookup has run, and a **10-minute trust window** (`_TRUST_MS`, > the poll interval) prevents it from downgrading a connection that recently served a real lookup (the bare GET can hit rate-limits/timeouts and falsely report `unreachable` even while lookups work — that was the «парсит но недоступен» bug). Passive poll is every **5 min** (not 60 s, to avoid hammering stat.gov) + on `window.focus` + on `sl:bridge-ready`. States: `is-online` (green «подключено») / `is-nologin` (amber «нужен вход по ЭЦП») / `is-unreachable` (amber «недоступен») / `is-offline` (red «нет связи») / `is-checking` (grey). `STATGOV_HEALTH` lives in **`chrome-extension/content.js` + `background.js`** (`statgovHealth()`) — needs an extension reload (manifest 0.3.0); the lookup-driven path works without any reload.
...
- **Signer defaults live in TWO places — keep them in sync**: `App.SIGNERS_DEFAULTS` + `App.SIGNERS_ROLES` (js/app.js, the editable form source of truth, rebuilt into `Utils.*` at runtime) and the hardcoded fallbacks in `js/utils.js` (`AS_MEMBERS`, `PRAVLENIE_MEMBERS`, `UPRAV_DIR_ROLE/NAME`, …). The АС council (`AS_MEMBERS`, used in АР/Протокол for non-pravlenie) / Правление (`PRAVLENIE_MEMBERS`, used for pravlenie АР/Протокол + as the СЗ-on-Правление approver via `UPRAV_DIR_*`) order = array order. Current defaults: Аринов Д.С. = «Заместитель Председателя Правления, член Правления» — АС position 3 (after Кныкова), Правление position 3 (**replaced** the Главбух Керн), and the СЗ-Правление approver; Директор ДАиП = **Бурханов Д.К.** (also `ar-form.js` `UNDERWRITER` for batch АР and the заключение signer). СЗ subjects: Правление → «О вынесении на рассмотрение Правления решения о заключении сделки[ с аффилированным лицом] (договор ОСРНС)»; СД → «…Совета директоров решения о заключении {крупной сделки | сделки с аффилированным лицом} (договора ОСРНС)» (see `sz-generator.js`).
