...
//   Автор ← Автор (Y, новый шаблон) → иначе Менеджер (старый шаблон)
//   Номер договора ← НомерДоговора (B)
...
//
// НОВЫЙ ШАБЛОН (52 колонки) добавил три поля; старый (49 колонок) читается как и
// раньше — колонки ищутся по тексту заголовка, поэтому сдвиг позиций не ломает парсер:
//   • ДатаДоговора (C) → dateContract. ПРИОРИТЕТНЕЕ номера договора: у части
//     договоров номер и фактическая дата расходятся (напр. номер 042407261200001
//     даёт 24.07.2026, а ДатаДоговора = 03.08.2026). Время «0:00:00» отбрасывается.
//     Эта дата идёт во ВСЕ документы (АР, Заключение, Протокол, СЗ), включая места
//     с датой прописью. Нет колонки (старый шаблон) → fallback на цифры номера [2..8).
//   • СтранаРезидентстваСтрахователя (F) → residencyCountry. «Казахстан» → резидент;
//     сверяется с авторитетным вердиктом egov (BatchAR._residRegDiff).
//   • Автор (Y) → author, отдельно от Менеджера (X).
//
...
const BatchReader = {
  ...
  FIELDS: {
    ...
    // Дата договора — ОТДЕЛЬНАЯ колонка нового шаблона («03.08.2026 0:00:00»).
    // Раньше дату выводили из цифр номера договора, но у части договоров номер и
    // фактическая дата расходятся, поэтому колонка приоритетнее номера.
    dateContractCol: { headers: ['датадоговора'], col: 'C' },
    ...
    // «Автор» приоритетнее «Менеджера»: в новом шаблоне это разные колонки, и в
    // таблице/документах показываем именно Автора. Старый шаблон — только Менеджер.
    author:         { headers: ['автор', 'менеджер'], col: 'Y' },
  ...
  parse(arrayBuffer) {
    ...
    for (let r = headerRow + 1; r < grid.length; r++) {
      ...
      rows.push({
        ...
        branch: '',
        // Дата договора: ПРИОРИТЕТ — колонка «ДатаДоговора» («03.08.2026 0:00:00»,
        // время отбрасываем). Если колонки нет (старый шаблон) — выводим из цифр
        // номера договора [2..8) = ДД.ММ.ГГ. Номер и фактическая дата могут
        // расходиться, поэтому колонка главнее. Эта дата идёт во ВСЕ документы.
        dateContract: BatchReader._date(cell(row, 'dateContractCol'))
          || BatchReader._contractDate(contractNumber),
        // Страна резидентства Страхователя из выгрузки («Казахстан» → резидент).
        residencyCountry: String(cell(row, 'residencyCountry') || '').trim(),
