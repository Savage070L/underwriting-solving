...
// (egov P30.01/P30.11 их тоже не проверяет, среди ИП есть и нерезиденты), поэтому такие
...
const ResidentCheck = {
  ...
  check(id) {
    ...
    if (kind === 'iin') {
      ...
      // (ГБД ЮЛ) к ним неприменим, egov P30.01/P30.11 их тоже не проверяет. Среди ИП есть
  ...
  // ===== АВТОРИТЕТНАЯ ПРОВЕРКА ЧЕРЕЗ egov (P30.01/P30.11, мост-расширение) =====
  // Локальный индекс gbd_ul отстаёт (нет свежих регистраций) → даёт ложных
  // нерезидентов. egov P30.01/P30.11 — источник резидентства (налоговый) и актуальнее.
  // Схема гибридная: локальный индекс = мгновенный ответ (оффлайн, без сети),
  // egov через мост = АВТОРИТЕТНОЕ уточнение.
  //
  // БЮДЖЕТ ЗАПРОСОВ. P30.01/P30.11 — именной запрос госуслуги под аккаунтом пользователя
  ...
  _egovRaw: new Map(),       // bin → сырой ответ P30.01/P30.11 (его же показывает досье)
  ...
  // Сырой ответ P30.01/P30.11 по БИН (для досье) — синхронно из кэша, без сети.
  egovRawFor(id) {
    ResidentCheck._egovStoreInit();
    return ResidentCheck._egovRaw.get(ResidentCheck._norm(id)) || null;
  },

  // ЕДИНСТВЕННАЯ точка сетевого запроса P30.01/P30.11. Promise<raw|null>: null — ИИН,
  // мост недоступен, действует пауза после ошибки или запрос не удался.
  fetchEgovRaw(id) {
    ResidentCheck._egovStoreInit();
    const s = ResidentCheck._norm(id);
    if (s.length !== 12) return Promise.resolve(null);
    // Эндпоинт /organizations — ТОЛЬКО для БИН юрлиц. Для ИИН (ИП/физлицо,
  ...
  // Ответ egov P30.01/P30.11 → verdict. АВТОРИТЕТНЫЙ признак — булев `resident`:
  ...
  _egovVerdict(d) {
    ...
    if (d.resident === true) {
      // Новый портал (fgw.egov.kz, услуги P3001/P3011) отдельным кодом сообщает, что
    ...
    return {
      ...
      title: `egov (P30.01/P30.11): нерезидент${note ? ' — ' + note : ''}`,
    };
  },
};
